...
@section('acciones', '')
@section('breadcumb')
  <li class="breadcrumb-item"><span class="text-muted">{{ $arreglo['breadcumb'] }}</span> </li>
@endsection
@section('style')
  ...
  <div class="card card-custom">
    <div class="card-header">
      <div class="card-title">
        ...
        <h3 class="card-label">{{ $arreglo['breadcumb'] }}</h3>
      </div>
      <div class="card-toolbar">
        <!--begin::Button-->
        <a href="/catalogos/{{ $catalogo }}/create" class="btn btn-primary font-weight-bolder">
    ...
    <div class="modal-dialog" role="document">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title" id="exampleModalLabel">Eliminar {{ strtolower($arreglo['label']) }}</h5>
        ...
        <form method="post">
          ...
          <div class="modal-body">
            Estás eliminando un{{ $arreglo['sexo'] == 'a' ? 'a' : '' }} {{ strtolower($arreglo['label']) }}, ésta acción no se podrá deshacer, ¿realmente quieres hacerlo?
          </div>
...
@section('script')
  ...
  $(function() {
    tabla = $('#kt_datatable').DataTable({
      processing: true,
      serverSide: true,
      order: {!! array_key_exists('orden', $arreglo) ? $arreglo["orden"] : '[[0, \'desc\']]' !!},
      ajax: {
        url: "/catalogos/{{ $catalogo }}/tabla",
      },
      columns: [
        @foreach ($arreglo["campos"] as $key => $value)
          @if ( !isset($value['hiddenTabla']) )
  ...
  function eliminar(id) {
    $('#modalEliminar form').attr('action', "/catalogos/{{ $catalogo }}/" + id + "/destroy");
